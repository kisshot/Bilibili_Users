...
class BilibiliSpider(scrapy.Spider):
    ...
    headers = {
        ...
        'Referer': 'https://space.bilibili.com/177770496?from=search&seid=' + str(random.randint(10000, 50000))
            #521401
    }
    #
    ...
    def start_requests(self):
        ...
        yield scrapy.http.FormRequest(self.user_url, method='POST', headers=self.headers, formdata={"mid":"177770496"}, callback=self.parse_user, dont_filter = True)
        ...
        for pn in range(1,6):
            yield scrapy.http.Request(self.fans_url.format(user=self.start_user, pn=pn), callback=self.parse_fans,
                                  dont_filter=True)
